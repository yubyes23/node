...
const detail = async id => {
    ...
    // 获取线路和播放列表
    const tabsAll = pdfa(html, '#tv_tab li a').map(a => (a.match(/>([^<]+)/)?.[1] || "线路").trim());
    ...
    // 处理线路名称（确保数量与播放列表一致）
    let tabs = tabsAll;
    if (tabsAll.length > urls.length) {
        tabs = tabsAll.slice(0, urls.length);
    } else if (tabsAll.length < urls.length && tabsAll.length > 0) {
        while (tabs.length < urls.length) {
            tabs.push("线路" + (tabs.length + 1));
        }
    }
    ...
    if (!vod_name) {
        ...
        // 移除常见的站点名称分隔符
        vod_name = vod_name.split('_')[0].split('-')[0].trim();
    }
    // 3. 如果仍失败，从 og:title meta 提取
    if (!vod_name) {
        vod_name = pdfh(html, 'meta[property="og:title"]@content') || '';
    }
    // 4. 清理名称（移除《》及多余字符）
    vod_name = vod_name.replace(/.*《(.+)》.*/, '$1').trim();
    if (!vod_name) vod_name = "未知片名";
